    ...
    <section className="py-16 sm:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10">
          ...
          <div className="hidden md:block absolute top-1/2 left-[18%] right-[18%] -translate-y-8 h-0.5 border-t-2 border-dashed border-slate-200 -z-0" />
          ...
          {steps.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className="relative z-10 flex flex-col items-center text-center p-6 sm:p-8 bg-brand-sand rounded-2xl border border-slate-100 shadow-subtle hover:shadow-card transition-all"
              >
                {/* Step indicator badge */}
                <span className="w-14 h-14 rounded-2xl bg-brand-navy text-brand-accent text-lg font-extrabold flex items-center justify-center font-serif shadow-md mb-6">
                  {item.step}
                </span>

                <div className="w-12 h-12 rounded-xl bg-orange-100 text-brand-accent flex items-center justify-center mb-4">
